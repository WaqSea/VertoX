...
def delete_self():
    ...
    time.sleep(2)  # Kod tamamen bitsin, sonra silinsin
    try:
        os.remove(path)
        print(f"{Fore.GREEN}setup.py başarıyla silindi.")
    except Exception as e:
        print(f"{Fore.RED}setup.py silinemedi: {e}")
...
def delete_setup_bat():
    ...
    if os.path.exists(bat_path):
        try:
            ...
            delete_self()
            print(Fore.YELLOW + f"{bat_path} başarıyla silindi.")
        except Exception as e:
            print(Fore.RED + f"{bat_path} silinemedi: {e}")
    else:
        print(Fore.RED + f"{bat_path} bulunamadı.")
